...
function usage() {
  ...
  chapter-url    Ví dụ: https://www.mgeko.cc/reader/en/murim-login-chapter-250-eng-li/
  fromChapter    Số chapter bắt đầu (ví dụ 230)
  toChapter      Số chapter kết thúc (ví dụ 250), bao gồm cả hai đầu
  output.json    Mặc định: chapters-mgeko-batch.json

  Ảnh lấy theo selector: #chapter-reader img
...
async function main() {
  ...
  if (!parsed) {
    console.error(
      "URL không khớp pattern ...-chapter-<số>-.../ (ví dụ ...-chapter-250-eng-li/)."
    );
    process.exit(1);
  }
  ...
  const outPath = resolve(process.cwd(), outArg || "chapters-mgeko-batch.json");
  const chapters = [];
  ...
  for (let n = fromChapter; n <= toChapter; n++) {
    ...
    if (!result.ok) {
      ...
      entry.error = "No images found (selector #chapter-reader img)";
    }
