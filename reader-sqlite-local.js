...
(function () {
      ...
      function buildDataFromSqlDb(db, fileLabel) {
        const probe = db.exec(
          "SELECT name FROM sqlite_master WHERE type='table' AND name IN ('chapters','chapter_images')"
        );
        const found = probe[0] ? probe[0].values.map((row) => row[0]) : [];
        if (!found.includes("chapters") || !found.includes("chapter_images")) {
          throw new Error(
            "File không phải DB bộ truyện (cần bảng chapters + chapter_images). Thường là catalog/db/series/ID.sqlite"
          );
        }
        ...
        while (stmt.step()) {
          ...
          chapters.push({
            ...
            error:
              r.fetch_ok === 1
                ? null
                : r.error_message || (r.image_count > 0 ? null : "Chưa có ảnh hoặc lỗi"),
          });
        }
        stmt.free();
        if (chapters.length === 0) {
          throw new Error("Database không có dòng chapter.");
        }
        const nums = chapters.map((c) => c.chapter);
        const base = String(fileLabel || "series.sqlite").replace(/\\/g, "/");
        const shortName = base.includes("/") ? base.split("/").pop() : base;
        return {
          title: shortName.replace(/\.sqlite$/i, ""),
          fromChapter: Math.min(...nums),
          toChapter: Math.max(...nums),
          fetchedAt: null,
          chapters,
        };
      }
      ...
      async function ensureChapterImagesLoaded(index) {
        if (!sqlDb || !data || index < 0 || index >= data.chapters.length) return;
        ...
        ch._loadPromise = Promise.resolve()
          .then(() => {
            ...
            while (stmt.step()) {
              const row = stmt.get();
              if (row && row[0]) urls.push(String(row[0]));
            }
            stmt.free();
            ch.images = urls;
            ch.total = urls.length;
          })
          .finally(() => {
      ...
      function renderChapter(token) {
...

        {
          ...
          if (activeIndex < data.chapters.length - 1) {
            ...
            nextBtn.textContent = "Quay về home";
            nextBtn.addEventListener("click", function () {
              if (window.ComicHubHistory && ComicHubHistory.clearAndNavigate) {
                ComicHubHistory.clearAndNavigate("home.html");
              } else {
      ...
      async function applyData(j, fileLabel) {
        data = j;
        manifestKey = "local:" + String(fileLabel || j.title || "db");
      ...
      let sqlJsCached = null;

      async function getSqlJs() {
        if (sqlJsCached) return sqlJsCached;
        if (typeof initSqlJs !== "function") {
          throw new Error(
            "Chưa có initSqlJs (sql.js). Kiểm tra thẻ script sql-wasm.js và mạng lần đầu để tải WASM."
          );
        }
        sqlJsCached = await initSqlJs({
          locateFile: (f) =>
       ...
       * @param {string} label
       */
      async function openSqliteFromBuffer(buf, label) {
        const SQL = await getSqlJs();
        let db;
        try {
          db = new SQL.Database(new Uint8Array(buf));
        } catch (e) {
          throw new Error("Không mở được SQLite: " + (e.message || String(e)));
        }
        try {
          if (sqlDb) {
            try {
          ...
          sqlDb = db;
          const j = buildDataFromSqlDb(db, label);
          await applyData(j, label);
        } catch (e) {
          try {
            db.close();
          } catch {
      ...
      async function openSqliteFile(file) {
        if (!file) return;
        try {
          removeChapterProgressBar();
          el.main.innerHTML = '<div class="empty glass">Đang mở SQLite trong trình duyệt…</div>';
          el.nav.innerHTML = "";
          const buf = await file.arrayBuffer();
          await openSqliteFromBuffer(buf, file.name);
        } catch (e) {
          showMainError(e.message || String(e));
        }
      }
      ...
      /**
       * Tải .sqlite qua HTTP (GitHub Pages / host tĩnh). Đường dẫn tương đối so với URL trang.
       */
      async function openSqliteFromRemotePath(relPath) {
        removeChapterProgressBar();
        el.main.innerHTML = '<div class="empty glass">Đang tải file SQLite…</div>';
        el.nav.innerHTML = "";
        const resolved = new URL(relPath, location.href).href;
        ...
        if (!res.ok) {
          throw new Error(
            "HTTP " +
              ...
              " — không tải được " +
              relPath +
              ". File phải nằm trên cùng site (cùng origin), ví dụ commit vào docs/ hoặc nhánh gh-pages."
          );
        }
        const buf = await res.arrayBuffer();
        const short = relPath.replace(/^.*\//, "") || "remote.sqlite";
        await openSqliteFromBuffer(buf, short);
      }
      ...
      function bindReaderChrome() {
        ...
        const pick = document.getElementById("sqliteFilePick");
        if (pick) {
          pick.addEventListener("change", function (ev) {
            const f = ev.target.files && ev.target.files[0];
            if (f) void openSqliteFile(f);
            pick.value = "";
          });
        }
        ...
        ["dragenter", "dragover"].forEach((evt) => {
          drop.addEventListener(evt, function (e) {
            e.preventDefault();
            e.stopPropagation();
            drop.classList.add("sqlite-drag-hover");
          });
        });
        ["dragleave", "drop"].forEach((evt) => {
          drop.addEventListener(evt, function (e) {
            e.preventDefault();
            e.stopPropagation();
            drop.classList.remove("sqlite-drag-hover");
          });
        });
        drop.addEventListener("drop", function (e) {
          const f = e.dataTransfer && e.dataTransfer.files && e.dataTransfer.files[0];
          if (f && /\.(sqlite|db)$/i.test(f.name)) void openSqliteFile(f);
        });

        const btn = document.getElementById("sqliteFileBtn");
        if (btn && pick) {
      ...
      function showPlaceholderHelp() {
        ...
        el.placeholder.innerHTML =
          "Chọn file <code>series/*.sqlite</code> hoặc kéo thả vào vùng đọc. " +
      ...
      const paramDb = currentParams().get("db");
      if (paramDb && paramDb.trim()) {
        void openSqliteFromRemotePath(paramDb.trim()).catch(function (e) {
          showMainError(e.message || String(e));
        });
      } else {
        showPlaceholderHelp();
